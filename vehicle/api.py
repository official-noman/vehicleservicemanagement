...
from .schemas import (
    UserOut,
    CustomerOut, CustomerRegistrationIn,
    MechanicOut, MechanicRegistrationIn, MechanicSalaryIn,
    RequestIn, RequestOut, AdminRequestIn, AdminApproveRequestIn,
    UpdateCostIn, MechanicUpdateStatusIn,
...
api = NinjaExtraAPI(title="Vehicle Service API", version="1.0.0")  #instead of ninja api
...
@api.post("/auth/register/customer", response={201: CustomerOut, 400: ErrorResponse}, tags=["Auth"])
...
@api.post("/auth/register/mechanic", response={201: MechanicOut, 400: ErrorResponse}, tags=["Auth"])
...
def login(request, payload: LoginIn):
    ...
    role = "customer" if _is_customer(user) else "mechanic" if _is_mechanic(user) else "admin"
...
@api.get("/admin/customers", response=List[CustomerOut], auth=JWTAuth(), tags=["Admin - Customers"])
def admin_list_customers(request):
    return models.Customer.objects.select_related("user").all()


@api.post("/admin/customers", response={201: CustomerOut, 400: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Customers"])
...
@api.put("/admin/customers/{customer_id}", response={200: CustomerOut, 404: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Customers"])
...
@api.delete("/admin/customers/{customer_id}", response={200: SuccessResponse, 404: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Customers"])
...
@api.get("/admin/mechanics", response=List[MechanicOut], auth=JWTAuth(), tags=["Admin - Mechanics"])
def admin_list_mechanics(request):
    return models.Mechanic.objects.select_related("user").all()


@api.get("/admin/mechanics/pending", response=List[MechanicOut], auth=JWTAuth(), tags=["Admin - Mechanics"])
def admin_list_pending_mechanics(request):
    return models.Mechanic.objects.select_related("user").filter(status=False)


@api.post("/admin/mechanics", response={201: MechanicOut, 400: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Mechanics"])
...
@api.patch("/admin/mechanics/{mechanic_id}/approve", response={200: MechanicOut, 404: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Mechanics"])
...
@api.put("/admin/mechanics/{mechanic_id}", response={200: MechanicOut, 404: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Mechanics"])
...
@api.patch("/admin/mechanics/{mechanic_id}/salary", response={200: MechanicOut, 404: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Mechanics"])
...
@api.delete("/admin/mechanics/{mechanic_id}", response={200: SuccessResponse, 404: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Mechanics"])
...
@api.get("/admin/requests", response=List[RequestOut], auth=JWTAuth(), tags=["Admin - Requests"])
def admin_list_requests(request):
    return models.Request.objects.all().order_by("-id")


@api.get("/admin/requests/pending", response=List[RequestOut], auth=JWTAuth(), tags=["Admin - Requests"])
def admin_list_pending_requests(request):
    return models.Request.objects.filter(status="Pending")


@api.post("/admin/requests", response={201: RequestOut, 400: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Requests"])
...
@api.patch("/admin/requests/{request_id}/approve", response={200: RequestOut, 404: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Requests"])
...
@api.patch("/admin/requests/{request_id}/status", response={200: RequestOut, 404: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Requests"])
def admin_change_request_status(request, request_id: int, payload: AdminApproveRequestIn):
...
@api.patch("/admin/requests/{request_id}/cost", response={200: RequestOut, 404: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Requests"])
...
@api.delete("/admin/requests/{request_id}", response={200: SuccessResponse, 404: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Requests"])
...
@api.get("/admin/invoices", response=List[InvoiceOut], auth=JWTAuth(), tags=["Admin - Reports"])
...
@api.get("/admin/reports", response=List[RequestOut], auth=JWTAuth(), tags=["Admin - Reports"])
...
@api.get("/admin/service-costs", response=List[RequestOut], auth=JWTAuth(), tags=["Admin - Reports"])
...
@api.post("/admin/attendance", response={201: SuccessResponse, 400: ErrorResponse}, auth=JWTAuth(), tags=["Admin - Attendance"])
def admin_take_attendance(request, payload: BulkAttendanceIn):
    ...
    if len(payload.statuses) != len(mechanics):
        return 400, {"detail": f"Expected {len(mechanics)} status values, got {len(payload.statuses)}."}
...
@api.get("/admin/attendance", response=List[AttendanceOut], auth=JWTAuth(), tags=["Admin - Attendance"])
...
@api.get("/admin/feedback", response=List[FeedbackOut], auth=JWTAuth(), tags=["Admin - Feedback"])
...
@api.get("/customer/dashboard", response={200: CustomerDashboardOut, 403: ErrorResponse}, auth=JWTAuth(), tags=["Customer"])
def customer_dashboard(request):
    if not _is_customer(request.user):
        return 403, {"detail": "Not authorized."}

    customer = models.Customer.objects.get(user_id=request.user.id)
    bill = models.Request.objects.filter(
        customer_id=customer.id
    ).filter(Q(status="Repairing Done") | Q(status="Released")).aggregate(Sum("cost"))
...
@api.get("/customer/requests", response={200: List[RequestOut], 403: ErrorResponse}, auth=JWTAuth(), tags=["Customer"])
...
@api.get("/customer/requests/all", response={200: List[RequestOut], 403: ErrorResponse}, auth=JWTAuth(), tags=["Customer"])
def customer_list_all_requests(request):
    if not _is_customer(request.user):
        return 403, {"detail": "Not authorized."}

    customer = models.Customer.objects.get(user_id=request.user.id)
    return 200, models.Request.objects.filter(customer_id=customer.id).exclude(status="Pending")


@api.post("/customer/requests", response={201: RequestOut, 403: ErrorResponse}, auth=JWTAuth(), tags=["Customer"])
...
@api.delete("/customer/requests/{request_id}", response={200: SuccessResponse, 403: ErrorResponse, 404: ErrorResponse}, auth=JWTAuth(), tags=["Customer"])
...
@api.get("/customer/profile", response={200: CustomerOut, 403: ErrorResponse}, auth=JWTAuth(), tags=["Customer"])
...
@api.put("/customer/profile", response={200: CustomerOut, 403: ErrorResponse}, auth=JWTAuth(), tags=["Customer"])
...
@api.get("/customer/invoice", response={200: List[RequestOut], 403: ErrorResponse}, auth=JWTAuth(), tags=["Customer"])
...
@api.post("/customer/feedback", response={201: SuccessResponse, 403: ErrorResponse}, auth=JWTAuth(), tags=["Customer"])
...
@api.get("/mechanic/dashboard", response={200: MechanicDashboardOut, 403: ErrorResponse}, auth=JWTAuth(), tags=["Mechanic"])
...
@api.get("/mechanic/work", response={200: List[RequestOut], 403: ErrorResponse}, auth=JWTAuth(), tags=["Mechanic"])
...
@api.patch("/mechanic/work/{request_id}/status", response={200: RequestOut, 403: ErrorResponse, 404: ErrorResponse}, auth=JWTAuth(), tags=["Mechanic"])
...
@api.get("/mechanic/attendance", response={200: List[AttendanceOut], 403: ErrorResponse}, auth=JWTAuth(), tags=["Mechanic"])
...
@api.get("/mechanic/salary", response={200: List[RequestOut], 403: ErrorResponse}, auth=JWTAuth(), tags=["Mechanic"])
def mechanic_salary(request):
    ...
    mechanic = models.Mechanic.objects.get(user_id=request.user.id)
    return 200, models.Request.objects.filter(
        mechanic_id=mechanic.id
    ).filter(Q(status="Repairing Done") | Q(status="Released"))


@api.get("/mechanic/profile", response={200: MechanicOut, 403: ErrorResponse}, auth=JWTAuth(), tags=["Mechanic"])
...
@api.put("/mechanic/profile", response={200: MechanicOut, 403: ErrorResponse}, auth=JWTAuth(), tags=["Mechanic"])
...
@api.post("/mechanic/feedback", response={201: SuccessResponse, 403: ErrorResponse}, auth=JWTAuth(), tags=["Mechanic"])
...
@api.post("/contact", response={200: SuccessResponse, 400: ErrorResponse}, tags=["Public"])
